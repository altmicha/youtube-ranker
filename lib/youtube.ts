...
/**
 * Fetches title, thumbnail, and channel name from the YouTube Data
 * API v3. Server-only — reads YOUTUBE_API_KEY from process.env, which
 * is never exposed to the browser since this file is only ever
 * imported from Server Actions/Components (nothing here is marked
 * "use client", and there's no NEXT_PUBLIC_ prefix on the env var).
 *
 ...
 * just the video ID, per the requirement that a metadata failure
